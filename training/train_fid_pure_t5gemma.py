"""
Pure FiD (Fusion-in-Decoder) Training Script with T5Gemma2
==========================================================

Applies the FiD algorithm (no source pointer) to T5Gemma2 backbone.

Paper: "Leveraging Passage Retrieval with Generative Models for Open Domain Question Answering"
Authors: Izacard & Grave (2021)

T5Gemma2 Adaptations:
- Model: T5Gemma2-270M-270M (540M total, 417M Vision Tower frozen)
- Tokenizer: AutoProcessor
- Optimizer: AdamW, lr=1e-5, Cosine decay
- BF16: Native support
- Vision Tower: Frozen (text-only task)
- decoder_start_token_id: Set to 2 (BOS) to prevent garbage output

Key differences from T5-base FiD:
- Hidden size: 1024 (vs 768)
- Requires transformers >= 5.0.0rc1

Usage:
    # Train with precomputed data
    python train_fid_pure_t5gemma.py \\
    ...
    # Quick test
    python train_fid_pure_t5gemma.py --quick_test --steps 100 \\
        --model_name /path/to/T5Gemma2-270M-270M \\
        --precomputed_path kilt_data/precomputed_fid/all_tasks_train.parquet

    # Resume from checkpoint
    python train_fid_pure_t5gemma.py --resume checkpoints/fid_pure_t5gemma/step_5000
"""
...
class FiDPureT5GemmaTrainer:
    """
    Pure FiD trainer with T5Gemma2 backbone (no source pointer, no compression).

    Key features:
    - T5Gemma2 model with Vision Tower frozen
    ...
    def _init_model(self) -> None:
        ...
        print(f"\n{'='*60}")
        print("Initializing FiD Pure with T5Gemma2 Backbone")
        print(f"{'='*60}")
        ...
        self.tokenizer = self.processor.tokenizer  # Extract tokenizer for compatibility

        # Load model with BF16 if enabled
        if self.args.bf16:
        ...
        # Fix decoder_start_token_id if not set (prevents garbage output)
        ...
        # Freeze Vision Tower (text-only task, ~417M parameters saved)
        vision_tower = None
        ...
        if vision_tower is not None:
            ...
            print("  No Vision Tower found (text-only model)")
    ...
    def _init_optimizer(self) -> None:
        ...
        Adafactor: FiD paper setting, lr=1e-3, adaptive
        AdamW: T5Gemma2 setting, lr=1e-5, with Cosine decay
        """
    ...
    def sample_precomputed(self) -> Optional[Dict[str, Any]]:
        ...
        Uses temperature-weighted task sampling.

        KEY: Uses 'answer' field as target (not 'target_text')
    ...
    def encode_and_fuse_batch(
        ...
        Args:
            batch_input_texts: List of [List of passages per query]
        ...
        n_passages = len(batch_input_texts[0])
        ...
        # FiD compression: first k vectors per passage
    ...
    def training_step_batch(self, batch: List[Dict[str, Any]]) -> float:
        ...
        # Use 'answer' as target (pure FiD format)
    ...
    def train(self) -> None:
        """Main training loop."""
        print(f"\n{'='*60}")
        print("Starting FiD Pure Training with T5Gemma2")
        print(f"{'='*60}")
    ...
    def load_checkpoint(self, checkpoint_path: str) -> None:
        ...
        # Load processor from base model (checkpoint doesn't save full processor files)
        # Use the original model_name from args
        ...
        if os.path.exists(optimizer_path):
            ...
            print(f"  Warning: optimizer.pt not found, using fresh optimizer")
    ...
    def _evaluate_realtime(self) -> Dict[str, float]:
        ...
        self.model.eval()
        results = {}
        tasks_to_eval = ["nq", "triviaqa_support_only"]
    ...
    def _save_config(self) -> None:
        ...
        config = {
            ...
            "num_beams": self.args.num_beams,
            "bf16": self.args.bf16,
            "paper_reference": "Izacard & Grave (2021) - FiD with T5Gemma2 backbone"
        }
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="FiD Pure Training with T5Gemma2 Backbone",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )

    # Model (T5Gemma2 settings)
    parser.add_argument("--model_name", type=str, required=True,
                        help="Path to T5Gemma2 model")
    parser.add_argument("--compression_k", type=int, default=250,
                        help="Vectors per passage (250 = no compression)")
    ...
    # Training (T5Gemma2 settings)
    parser.add_argument("--total_steps", type=int, default=10000,
                        help="Total training steps")
    parser.add_argument("--gradient_accumulation_steps", type=int, default=64,
                        help="Gradient accumulation (effective batch=64)")
    parser.add_argument("--optimizer", type=str, default="adamw", choices=["adafactor", "adamw"],
                        help="Optimizer: adafactor (lr=1e-3) or adamw (lr=1e-5)")
    parser.add_argument("--learning_rate", type=float, default=1e-5,
                        help="Learning rate (1e-3 for adafactor, 1e-5 for adamw)")
    parser.add_argument("--weight_decay", type=float, default=0.01,
                        help="AdamW weight decay (ignored for Adafactor)")
    ...
    parser.add_argument("--num_beams", type=int, default=1,
                        help="Beam size for evaluation (1 = greedy)")

    # Checkpointing
    parser.add_argument("--output_dir", type=str, default="checkpoints/fid_pure_t5gemma",
    ...
    parser.add_argument("--bf16", action="store_true", default=True,
                        help="Use BF16 mixed precision (default: True for T5Gemma2)")
...
def main():
    ...
    print(f"\n{'='*60}")
    print("FiD Pure Training with T5Gemma2 Backbone")
    print(f"{'='*60}")
    ...
    # Initialize trainer
    trainer = FiDPureT5GemmaTrainer(args)
