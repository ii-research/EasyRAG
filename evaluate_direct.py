"""
Direct Baseline Evaluation Script (No RAG)
==========================================

Evaluates a model trained without any retrieval - tests model's parametric knowledge.
...
Usage:
    ...
    python evaluate_direct.py --checkpoint checkpoints/direct_t5base/step_10000 \
    ...
    # Verbose output
    python evaluate_direct.py --checkpoint checkpoints/direct_t5base/step_10000 \
        --data_path kilt_data/precomputed_v5/all_tasks_dev.parquet --verbose
"""
...
class DirectEvaluator:
    """
    Direct baseline evaluator (no retrieval, question-only input).
    ...
    def _load_model(self) -> None:
        ...
        self.tokenizer = T5Tokenizer.from_pretrained(self.args.checkpoint)
        self.model = T5ForConditionalGeneration.from_pretrained(self.args.checkpoint)
        ...
        if os.path.exists(config_path):
            ...
            print(f"Loaded config: algorithm={self.config.get('algorithm', 'direct')}")
        else:
            self.config = {}
    ...
    def generate(self, query: str) -> str:
        """Generate answer from question only (no context)."""
    ...
    def evaluate_task(
        ...
        for sample in pbar:
            ...
            # Generate (question only, no context)
            pred_answer = self.generate(query)
    ...
    def evaluate(self) -> Dict[str, Dict[str, float]]:
        ...
        print(f"\n{'='*60}")
        print("Direct Baseline Evaluation (No RAG, Question-Only)")
        print(f"{'='*60}")
    ...
    def _save_results(self, results: Dict[str, Dict[str, float]]) -> None:
        ...
        # Save to output path or checkpoint directory
        if self.args.output:
            results_path = self.args.output
        else:
            results_path = os.path.join(self.args.checkpoint, "eval_results_direct.json")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Direct Baseline Evaluation (No RAG)",
...
def main():
    ...
    evaluator = DirectEvaluator(args)
