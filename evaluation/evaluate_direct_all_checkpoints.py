"""
Batch evaluation script for Direct baseline checkpoints (Parallel version).

Evaluates all step_* checkpoints in parallel (one checkpoint per GPU).

Usage:
    python evaluate_direct_all_checkpoints.py \
        --checkpoint_dir checkpoints/direct_t5base \
...
def evaluate_single_checkpoint(args_tuple):
    ...
    cmd = [
        sys.executable, "evaluate_direct.py",
...
def main():
    parser = argparse.ArgumentParser(
        description="Batch evaluation of Direct baseline checkpoints (parallel)"
    )
    ...
    print(f"\n{'='*80}")
    print("SUMMARY: Direct Baseline All Checkpoint Results")
    print(f"{'='*80}")
