"""
Batch Evaluation Script for Stochastic RAG T5Gemma2 Checkpoints
================================================================

Evaluates all Stochastic RAG T5Gemma2 checkpoints in parallel (one checkpoint per GPU).
Generates plots showing loss curve, E[U] curve, and eval metrics over training.

Usage:
    python evaluate_stochastic_rag_t5gemma_all_checkpoints.py \
        --checkpoint_dir $DATA_DIR/checkpoints/stochastic_rag_t5gemma_v6 \
        --data_path $KILT_DATA_DIR/precomputed_v5/all_tasks_dev.parquet \
        --output_dir results/stochastic_rag_t5gemma_v6_all_checkpoints \
...
def evaluate_single_checkpoint(args_tuple):
    ...
    checkpoint_path, data_path, output_path, gpu_id, num_beams, num_passages = args_tuple
    ...
    # Use T5Gemma2 evaluation script with bf16
    cmd = [
        sys.executable, "evaluate_stochastic_rag_t5gemma.py",
...
def plot_eval_progress(
    ...
    plt.suptitle('Stochastic RAG T5Gemma2 Evaluation Progress', fontsize=14)
    plt.tight_layout()
...
def plot_training_progress(
    ...
    plt.suptitle('Stochastic RAG T5Gemma2 Training Progress', fontsize=14)
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches='tight')
    plt.close()
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate all Stochastic RAG T5Gemma2 checkpoints")
    parser.add_argument(
        "--checkpoint_dir",
        type=str,
        required=True,
        help="Directory containing Stochastic RAG T5Gemma2 checkpoints",
    )
    parser.add_argument(
        "--data_path",
        type=str,
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="results/stochastic_rag_t5gemma_all_checkpoints",
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Number of passages to use (default: 20 for T5Gemma2)",
    )
    parser.add_argument(
        "--max_samples",
        type=int,
    ...
    if args.start_step or args.end_step:
        ...
        print(f"Found {len(checkpoints)} Stochastic RAG T5Gemma2 checkpoints in range {range_str}:")
    else:
        print(f"Found {len(checkpoints)} Stochastic RAG T5Gemma2 checkpoints:")
    ...
    if not tasks:
        ...
        tasks_with_gpu = [
            (cp, dp, op, i % num_gpus, args.num_beams, args.num_passages)
    ...
    # Print summary table
    print("\n" + "=" * 100)
    print("SUMMARY: Stochastic RAG T5Gemma2 All Checkpoint Results")
    print("=" * 100)
