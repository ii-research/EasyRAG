"""
Direct Baseline Training Script (No RAG)
========================================

Trains a model without any retrieval - purely relies on model's parametric knowledge.

- Direct: No retrieval, only question encoding, outputs answer
- Used as lower-bound baseline to show the value of retrieval
...
Key characteristics:
- No passages/context used
- Tests model's internal knowledge
- Expected to perform worse than RAG methods on knowledge-intensive tasks

Usage:
    # Train from scratch
    python train_direct.py \
        --precomputed_path kilt_data/precomputed_v5/all_tasks_train.parquet \
        --output_dir checkpoints/direct_t5base

    # Quick test
    python train_direct.py --quick_test --steps 100 \
        --precomputed_path kilt_data/precomputed_v5/all_tasks_train.parquet
"""
...
class DirectTrainer:
    """
    Direct baseline trainer (no retrieval, question-only input).

    Key features:
    - No passages or context
    - Question -> Answer training
    - Tests model's parametric knowledge
    """
    ...
    def _init_model(self) -> None:
        ...
        print(f"\n{'='*60}")
        print("Initializing Direct Baseline (No RAG)")
        print(f"{'='*60}")
    ...
    def _init_data(self) -> None:
        ...
        print(f"\n{'='*60}")
        print("Initializing Data Pipeline (Direct - No Passages)")
        print(f"{'='*60}")

        self._init_precomputed_data()

    def _init_precomputed_data(self) -> None:
        """Load precomputed data for training (only query and answer)."""
        import pandas as pd
    ...
    def sample_precomputed(self) -> Optional[Dict[str, Any]]:
        """Sample a training example (only query and answer, no passages)."""
        ...
        # Direct baseline: only query and answer
        return {
    ...
    def training_step_batch(self, batch: List[Dict[str, Any]]) -> float:
        ...
        Direct baseline: input is just "question: {query}"
        Target is just the answer.
        """
        self.model.train()

        # Format inputs: "question: {query}"
        input_texts = [f"question: {sample['query']}" for sample in batch]
    ...
    def train(self) -> None:
        """Main training loop."""
        print(f"\n{'='*60}")
        print("Starting Direct Baseline Training (No RAG)")
        print(f"{'='*60}")
        ...
        for task, samples in self.precomputed_val_data.items():
            ...
            with torch.no_grad():
                for sample in tqdm(eval_samples, desc=f"  {task}", leave=False):
                    ...
                    # Direct: only question, no context
    ...
    def _save_config(self) -> None:
        ...
        config = {
            "model_name": self.args.model_name,
            "algorithm": "direct",
            "description": "No RAG baseline - question only, no retrieval",
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Direct Baseline Training (No RAG)",
    ...
    parser.add_argument("--max_input_length", type=int, default=512,
                        help="Max input tokens (question only)")
    ...
    # Checkpointing
    parser.add_argument("--output_dir", type=str, default="checkpoints/direct_t5base",
...
def main():
    ...
    print(f"\n{'='*60}")
    print("Direct Baseline Training (No RAG)")
    print(f"{'='*60}")
