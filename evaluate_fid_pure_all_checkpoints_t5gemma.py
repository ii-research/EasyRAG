"""
Batch evaluation script for Pure FiD T5Gemma2 checkpoints (Parallel version).

Evaluates all step_* checkpoints in parallel (one checkpoint per GPU).

Usage:
    python evaluate_fid_pure_all_checkpoints_t5gemma.py \
        --checkpoint_dir checkpoints/fid_pure_t5gemma \
...
def evaluate_single_checkpoint(args_tuple):
    ...
    cmd = [
        sys.executable, "evaluate_fid_pure_t5gemma.py",
    ...
    results_path = os.path.join(checkpoint_path, "eval_results_pure_fid_t5gemma.json")
...
def main():
    parser = argparse.ArgumentParser(
        description="Batch evaluation of Pure FiD T5Gemma2 checkpoints (parallel)"
    )
    ...
    # Print summary
    print(f"\n{'='*80}")
    print("SUMMARY: Pure FiD T5Gemma2 All Checkpoint Results")
    print(f"{'='*80}")
    ...
    combined_path = os.path.join(args.output_dir, "all_eval_results_t5gemma.json")
