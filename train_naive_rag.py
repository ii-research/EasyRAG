"""
Naive RAG Baseline Training Script
==================================

Trains a model with simple RAG: concatenate all passages into one context.
...
Key characteristics:
- All passages concatenated into single context
- Limited by max input length (512 tokens for T5-base)
- Uses standard encoder-decoder (NOT Fusion-in-Decoder)

Usage:
    # Train from scratch
    python train_naive_rag.py \
        --precomputed_path kilt_data/precomputed_v5/all_tasks_train.parquet \
        --output_dir checkpoints/naive_rag_t5base \
        --num_passages 10

    # Quick test
    python train_naive_rag.py --quick_test --steps 100 \
        --precomputed_path kilt_data/precomputed_v5/all_tasks_train.parquet
"""
    ...
    try:
        ...
        update_step_state(
            ...
            extra={"loss": loss, "lr": lr, "global_step": global_step,
                   "algorithm": "naive_rag", "model": "t5base"}
        )
    except Exception:
        pass


class NaiveRAGTrainer:
    """
    Naive RAG trainer (concatenate passages, standard encoder-decoder).

    Key features:
    - All passages concatenated into single context string
    - Limited by model's max input length (512 tokens)
    - Standard T5 encoder-decoder (NOT Fusion-in-Decoder)
    """
    ...
    def _init_model(self) -> None:
        ...
        print(f"\n{'='*60}")
        print("Initializing Naive RAG Baseline (Concatenated Passages)")
        print(f"{'='*60}")
    ...
    def _extract_context_from_input_text(self, input_text: str) -> str:
        ...
        Input: "query: What is X? index: 1 context: Some passage text"
        Output: "Some passage text"
        """
        import re
        # Extract context after "context:" or "title: ... context:"
    ...
    def _format_naive_rag_input(self, query: str, input_texts: List[str]) -> str:
        ...
        Format:
            ...
            ...
        """
        # Take only num_passages passages
        passages = input_texts[:self.args.num_passages]

        # Extract context from each passage
        contexts = []
    ...
    def sample_precomputed(self) -> Optional[Dict[str, Any]]:
        ...
        # Format as Naive RAG input (concatenated passages)
    ...
    def training_step_batch(self, batch: List[Dict[str, Any]]) -> float:
        ...
        Naive RAG: input is "question: {query}\ncontext: [1] {p1}\n[2] {p2}..."
        Target is just the answer.
        Uses standard encoder-decoder (NOT Fusion-in-Decoder).
        """
        self.model.train()

        # Get formatted inputs (already concatenated passages)
        input_texts = [sample['formatted_input'] for sample in batch]
        target_texts = [sample["answer"] for sample in batch]

        # Tokenize inputs - will be truncated to max_input_length
        ...
        # Forward pass - standard encoder-decoder
        if self.args.bf16:
    ...
    def train(self) -> None:
        """Main training loop."""
        print(f"\n{'='*60}")
        print("Starting Naive RAG Baseline Training")
        print(f"{'='*60}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Naive RAG Baseline Training (Concatenated Passages)",
    ...
    parser.add_argument("--max_input_length", type=int, default=512,
                        help="Max input tokens (includes question + all passages)")
    ...
    # Checkpointing
    parser.add_argument("--output_dir", type=str, default="checkpoints/naive_rag_t5base",
...
def main():
    ...
    print(f"\n{'='*60}")
    print("Naive RAG Baseline Training (Concatenated Passages)")
    print(f"{'='*60}")
