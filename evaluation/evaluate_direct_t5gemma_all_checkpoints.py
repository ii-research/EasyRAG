"""
Batch evaluation script for Direct T5Gemma2 baseline checkpoints (Parallel version).

Evaluates all step_* checkpoints in parallel (one checkpoint per GPU).

Usage:
    python evaluate_direct_t5gemma_all_checkpoints.py \
        --checkpoint_dir checkpoints/direct_t5gemma_v1 \
...
def evaluate_single_checkpoint(args_tuple):
    ...
    checkpoint_path, val_path, output_path, gpu_id, max_samples = args_tuple
    ...
    cmd = [
        sys.executable, "evaluate_direct_t5gemma.py",
...
def main():
    parser = argparse.ArgumentParser(
        description="Batch evaluation of Direct T5Gemma2 baseline checkpoints (parallel)"
    )
    ...
    if not tasks:
        ...
        tasks_with_gpu = [
            (cp, val, out, i % num_gpus, args.max_samples)
    ...
    print(f"\n{'='*80}")
    print("SUMMARY: Direct T5Gemma2 Baseline All Checkpoint Results")
    print(f"{'='*80}")
