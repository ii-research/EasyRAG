"""
Direct Baseline Training Script with T5Gemma2 (No RAG)
======================================================

Trains a T5Gemma2 model without any retrieval - purely relies on model's parametric knowledge.
...
Usage:
    # Train from scratch
    python train_direct_t5gemma.py \
        --model_name /path/to/T5Gemma2-270M-270M \
        --precomputed_path kilt_data/precomputed_v5/all_tasks_train.parquet \
        --output_dir checkpoints/direct_t5gemma

    # Quick test
    python train_direct_t5gemma.py --quick_test --steps 100 \
        --model_name /path/to/T5Gemma2-270M-270M \
        --precomputed_path kilt_data/precomputed_v5/all_tasks_train.parquet
"""
    ...
    try:
        ...
        update_step_state(
            ...
            extra={"loss": loss, "lr": lr, "global_step": global_step,
                   "algorithm": "direct", "model": "t5gemma"}
        )
    except Exception:
        pass


class DirectT5GemmaTrainer:
    """
    Direct baseline trainer with T5Gemma2 (no retrieval, question-only input).
    ...
    def _init_model(self) -> None:
        ...
        print(f"\n{'='*60}")
        print("Initializing Direct Baseline with T5Gemma2 (No RAG)")
        print(f"{'='*60}")
    ...
    def train(self) -> None:
        """Main training loop."""
        print(f"\n{'='*60}")
        print("Starting Direct Baseline Training with T5Gemma2 (No RAG)")
        print(f"{'='*60}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Direct Baseline Training with T5Gemma2 (No RAG)",
    ...
    # Checkpointing
    parser.add_argument("--output_dir", type=str, default="checkpoints/direct_t5gemma",
...
def main():
    ...
    print(f"\n{'='*60}")
    print("Direct Baseline Training with T5Gemma2 (No RAG)")
    print(f"{'='*60}")
    ...
    trainer = DirectT5GemmaTrainer(args)
