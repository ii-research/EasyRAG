"""
Batch evaluation script for Naive RAG checkpoints (Parallel version).

Evaluates all step_* checkpoints in parallel (one checkpoint per GPU).

Usage:
    python evaluate_naive_rag_all_checkpoints.py \
        --checkpoint_dir checkpoints/naive_rag_t5base \
...
def evaluate_single_checkpoint(args_tuple):
    ...
    cmd = [
        sys.executable, "evaluate_naive_rag.py",
...
def main():
    parser = argparse.ArgumentParser(
        description="Batch evaluation of Naive RAG checkpoints (parallel)"
    )
    ...
    print(f"\n{'='*80}")
    print("SUMMARY: Naive RAG All Checkpoint Results")
    print(f"{'='*80}")
