"""
FiD-Light Training Script with T5Gemma2 Backbone
=================================================

Based on FiD-Light paper: Hofstatter et al. (2023)
Backbone: T5Gemma2 - Zhang et al. (2025)

This script adapts FiD-Light to use T5Gemma2-270M-270M as the generator backbone
instead of T5-Base, exploring the performance of next-generation encoder-decoder
models on RAG tasks.

Key differences from train_fidlight_paper.py:
...
Hyperparameters (following FiD-Light paper Appendix A):
...
Usage:
    ...
    python train_fidlight_t5gemma.py --precomputed_path kilt_data/precomputed_v5/all_tasks_train.parquet

    # Quick test (100 steps)
    python train_fidlight_t5gemma.py --quick_test --steps 100

    # Full training (50K steps)
    python train_fidlight_t5gemma.py --output_dir checkpoints/fidlight_t5gemma_270m

    # Resume from checkpoint
    python train_fidlight_t5gemma.py --resume checkpoints/fidlight_t5gemma_270m/step_10000

    # Multi-GPU training
    python train_fidlight_t5gemma.py --multi_gpu --batch_size 4
"""
    ...
    try:
        ...
        update_step_state(
            ...
            extra={"loss": loss, "lr": lr, "global_step": global_step,
                   "algorithm": "fidlight", "model": "t5gemma"}
        )
    except Exception:
...
class FiDLightT5GemmaTrainer:
    """
    FiD-Light trainer with T5Gemma2 backbone.

    Key features:
    - T5Gemma2-270M-270M as generator (540M params vs T5-base's 223M)
    ...
    def _init_model(self) -> None:
        ...
        print(f"\n{'='*60}")
        print("Initializing FiD-Light with T5Gemma2 Backbone (V10)")
        print(f"{'='*60}")

        model_name = self.args.model_name
        print(f"Loading {model_name}...")
        print(f"  (Requires transformers>=5.0.0rc1)")

        # Use AutoProcessor (official way for T5Gemma2)
        self.processor = AutoProcessor.from_pretrained(model_name)
        self.tokenizer = self.processor.tokenizer  # Keep tokenizer reference for compatibility

        # Load model with BF16 by default (T5Gemma2 is designed for BF16)
        if self.args.bf16:
        ...
        # ===== Freeze Vision Tower (not used for text-only FiD-Light) =====
        # T5Gemma2 structure: model.model.encoder.vision_tower (SiglipVisionModel)
        # self.model is T5Gemma2ForConditionalGeneration
        # self.model.model is T5Gemma2Model
        # self.model.model.encoder.vision_tower is SiglipVisionModel (~417M params)
        vision_tower = None
        ...
        if vision_tower is not None:
            ...
            print(f"  WARNING: Vision tower not found, check model structure")

        # ===== V10: Simple format + lr=1e-5 + cosine decay =====
        # - No chat template wrapping - direct "index: X text: Y" format
        # - decoder_start_token_id = 2 (BOS)
        # - lr=1e-5 (prevents training oscillation found in diagnostic)
        # - Cosine decay scheduler (already implemented)
        print(f"  V10: Simple format + lr=1e-5 + cosine decay")
    ...
    def extract_first_answer(self, generated_text: str) -> str:
        ...
        """
        import re
        # V9: No chat template, just strip
        text = generated_text.strip()

        # Find the first complete "index: X text: Y" pattern
        # Pattern: index: followed by numbers/commas, then text: followed by answer
        ...
        if "text:" in text:
            # Get everything after the first "text:" until next "index:" or end
            parts = text.split("text:", 1)
            if len(parts) > 1:
                answer_part = parts[1].split("index:")[0].strip()
                # Try to find index before text:
    ...
    def _get_encoder(self):
        ...
        # Fallback to get_encoder() if available
            ...
            raise AttributeError(
                f"Cannot access encoder from {type(base_model).__name__}. "
                "Model must have .encoder attribute or get_encoder() method."
            )

    def _init_optimizer(self) -> None:
        """Initialize optimizer (Adafactor or AdamW)."""
        print(f"\nInitializing {self.args.optimizer} optimizer...")
        ...
        if self.args.optimizer == "adafactor":
            # Adafactor (FiD-Light paper setting)
            # - Adaptive learning rate
            # - No weight decay needed
            # - Works well with lr=1e-3
            from transformers import Adafactor
            self.optimizer = Adafactor(
                self.model.parameters(),
                lr=self.args.learning_rate,
                scale_parameter=False,  # Don't scale lr by parameter size
                relative_step=False,    # Use fixed lr (not adaptive)
                warmup_init=False,      # We handle warmup separately
            )
            print(f"  Adafactor: scale_parameter=False, relative_step=False")
        else:
            # AdamW (T5Gemma2 paper setting)
    ...
    def encode_and_compress_batch(
        ...
        """
        Encode and compress a BATCH of samples using T5Gemma2 encoder.
    ...
    def training_step_batch(self, batch: List[Dict[str, Any]]) -> float:
        ...
        # ===== V10: Simple format like T5-Base =====
        # No chat template wrapping - direct "index: X text: Y" format
        # Add EOS token to targets so model learns when to stop
        # Target format: BOS + text + EOS (BOS added by tokenizer, EOS added manually)
        ...
        # ===== Train verbose: show input/target for debugging =====
        if getattr(self.args, 'train_verbose', 0) > 0 and self.global_step < self.args.train_verbose:
            print(f"\n{'='*60}")
            print(f"[Train Verbose] Step {self.global_step + 1}")
            print(f"{'='*60}")
            for i, sample in enumerate(batch[:2]):  # Show first 2 samples
                print(f"\n--- Sample {i+1} ---")
                ...
                # Show tokenized target
                target_tokens = target_inputs["input_ids"][i].tolist()
                target_decoded = self.tokenizer.decode(target_tokens, skip_special_tokens=False)
                print(f"Target tokens ({len(target_tokens)}): {target_tokens[:20]}...")
                print(f"Target decoded: {target_decoded[:150]}...")
                # Show first input passage
                if sample.get("input_texts"):
                    print(f"Input[0] (first 200 chars): {sample['input_texts'][0][:200]}...")
            print(f"{'='*60}\n")

        # Forward pass with optional BF16 mixed precision
        # Note: Only pass labels, let model handle decoder_input_ids automatically
        if self.args.bf16:
            # BF16 mixed precision (T5Gemma2 is BF16-native)
    ...
    def train(self) -> None:
        """Main training loop."""
        print(f"\n{'='*60}")
        print("Starting FiD-Light Training with T5Gemma2")
        print(f"{'='*60}")
        ...
        while self.global_step < self.args.total_steps:
            ...
            if self.accumulated_steps % self.args.gradient_accumulation_steps == 0:
                ...
                # Update progress (set_postfix first with refresh=False, then update)
                pbar.set_postfix({
                    "loss": f"{avg_loss:.4f}",
        ...
        for task, samples in self.precomputed_val_data.items():
            ...
            with torch.no_grad():
                for sample in tqdm(eval_samples, desc=f"  {task}", leave=False):
                    ...
                    # No prefix, let model generate from scratch using decoder_start_token_id
                    ...
                    # Optionally extract only the first answer
                    if getattr(self.args, 'extract_first_answer', False):
                        full_output = self.extract_first_answer(full_output)

                    # Extract answer (remove source pointer prefix)
                    # Bug fix: use split("text:", 1)[1] to get FIRST text: content
                    # Then split by "index:" to remove repeated patterns
                    if "text:" in full_output:
                        first_answer = full_output.split("text:", 1)[1]  # Get content after FIRST "text:"
                        first_answer = first_answer.split("index:")[0]   # Remove repeated "index:..." patterns
                        pred_answer = first_answer.strip().lower()
                    else:
                        pred_answer = full_output.lower().strip()
                    ...
                    if getattr(self.args, 'eval_verbose', 0) > 0 and total <= self.args.eval_verbose:
                        status = "✓" if is_correct else "✗"
                        print(f"\n  [{task}] Sample {total} {status}")
                        print(f"    Query: {sample.get('query', input_texts[0][:80])}...")
                        print(f"    Raw output: {full_output[:150]}...")
        ...
        for task in tasks_to_eval:
            ...
            with torch.no_grad():
                for sample in val_samples[:self.args.eval_samples]:
                    ...
                    # Bug fix: use split("text:", 1)[1] to get FIRST text: content
                    if "text:" in full_output:
                        first_answer = full_output.split("text:", 1)[1]
                        first_answer = first_answer.split("index:")[0]
                        pred_answer = first_answer.strip()
                    else:
                        pred_answer = full_output
    ...
    def _save_config(self) -> None:
        ...
        config = {
            ...
            "num_beams": self.args.num_beams,
            "bf16": self.args.bf16,
            "paper_reference": "FiD-Light: Hofstatter et al. (2023), T5Gemma2: Zhang et al. (2025)"
        }
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="FiD-Light Training with T5Gemma2 Backbone",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )

    # Model
    parser.add_argument("--model_name", type=str, default="google/t5gemma-2-270m-270m",
                        help="T5Gemma2 model name (google/t5gemma-2-270m-270m, google/t5gemma-2-1b-1b, google/t5gemma-2-4b-4b)")
    ...
    parser.add_argument("--optimizer", type=str, default="adafactor", choices=["adafactor", "adamw"],
                        help="Optimizer: adafactor (FiD-Light paper, lr=1e-3) or adamw (T5Gemma2 paper, lr=1e-5)")
    parser.add_argument("--learning_rate", type=float, default=1e-3,
                        help="Learning rate (1e-3 for adafactor, 1e-5 for adamw)")
    parser.add_argument("--warmup_steps", type=int, default=100,
                        help="Learning rate warmup steps")
    parser.add_argument("--max_grad_norm", type=float, default=1.0,
                        help="Max gradient norm for clipping")
    parser.add_argument("--weight_decay", type=float, default=0.01,
                        help="Weight decay for AdamW optimizer (ignored for Adafactor)")
    ...
    parser.add_argument("--extract_first_answer", action="store_true",
    ...
    parser.add_argument("--eval_verbose", type=int, default=0,
                        help="Print this many sample outputs during eval (0=disabled)")
    parser.add_argument("--train_verbose", type=int, default=0,
                        help="Print input/target for first N training steps (0=disabled)")

    # Checkpointing
    parser.add_argument("--output_dir", type=str, default="checkpoints/fidlight_t5gemma_270m",
...
def main():
    ...
    print(f"\n{'='*60}")
    print("FiD-Light with T5Gemma2 Backbone")
    print(f"{'='*60}")
    ...
    # Initialize trainer
    trainer = FiDLightT5GemmaTrainer(args)
