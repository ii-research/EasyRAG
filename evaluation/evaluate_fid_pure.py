"""
Pure FiD Evaluation Script
==========================

Evaluates pure FiD models (no source pointer prediction).
...
Usage:
    ...
    python evaluate_fid_pure.py --checkpoint checkpoints/fid_pure/step_50000

    # Quick test
    python evaluate_fid_pure.py --checkpoint checkpoints/fid_pure/step_50000 --max_samples 50

    # Verbose output
    python evaluate_fid_pure.py --checkpoint checkpoints/fid_pure/step_50000 --verbose
"""
...
class FiDPureEvaluator:
    """
    Pure FiD evaluator (no source pointer, answer-only output).
    ...
    def evaluate(self) -> Dict[str, Dict[str, float]]:
        ...
        """
        print(f"\n{'='*60}")
        print("Pure FiD Evaluation (Answer-Only, No Provenance)")
        print(f"{'='*60}")
        ...
        print(f"Strip index: {self.args.strip_index}")
        print()
    ...
    def _save_results(self, results: Dict[str, Dict[str, float]]) -> None:
        ...
        results_path = os.path.join(self.args.checkpoint, "eval_results_pure_fid.json")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Pure FiD Evaluation (No Source Pointer)",
    ...
    parser.add_argument("--max_input_length", type=int, default=250,
                        help="Max input tokens per passage (FiD paper: 250)")
    ...
    parser.add_argument("--strip_index", action="store_true",
                        help="Strip 'index: X' from input texts")
...
def main():
    ...
    evaluator = FiDPureEvaluator(args)
